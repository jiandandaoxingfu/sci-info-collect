...
function wheel_handle( delta, ele ) {
    ...
    if( ele.id && ele.id === 'painter' ) {
        let transform = ele.style.transform;
        let scale = parseFloat( transform.replace(/.*?scale\((.*?)\).*/, '$1') ) + delta;
