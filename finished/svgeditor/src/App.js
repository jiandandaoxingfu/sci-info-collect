...
function wheel_handle( delta, ele ) {
 ...
 //    let pp = p.parentNode;
	// if( p.tagName && p.id === 'painter' ) {
 //        ele = p;
 //    } else if( p.id === 'axis' || p.id === 'grid' ) {
 //        ele = pp;
 //    }
...
window.onload = () => {
    ...
    // setInterval( () => {
    //     if( canvas.isDrawing || editor.editor || drager.ele ) return;
    //     let str = canvas.canvas.innerHTML;
    //     let n = history_list.length;
    //     if( str !== history_list[n - 1] ) {
    //         if( n > 20 ) history_list = history_list.slice(1, n);
    ...
    document.addEventListener('mousedown', (e) => {
        ...
        let p = ele.parentNode;
        if(p.id === 'axis' || p.id === 'grid' ) {
            if (action === 'move') drager.ele = $$('painter');
        }
    ...
    document.addEventListener('mouseup', (e) => {
        ...
        if (tagName == 'button') {
            ...
            switch (action) {
                // case '撤销':
                //     idx = history_list.indexOf(canvas.canvas.innerHTML);
                //     if( idx === 0 ) {
                //         alert('无法继续撤回');
                //     } else if( idx > 0 ) {
                //         canvas.canvas.innerHTML = history_list[idx - 1];
                //     }
                //     break;
                // case '重做':
                //     idx = history_list.indexOf(canvas.canvas.innerHTML) + 1;
                //     if( idx == history_list.length ) {
                //         alert('已经到头了！！！');
                //     } else {
                ...
                case '导出':
                    let fn = prompt('输入文件名： ', '');
                    if( fn ) svg2image(canvas.canvas, fn, 800, 500, 'png');
                    break;
                case '坐标轴':
        ...
        if( canvas.isDrawing ) { 
            if( canvas.isMove ) {
                editor.createEditor(canvas.currentEle.obj);
            }
            canvas.mouseup();
        }
